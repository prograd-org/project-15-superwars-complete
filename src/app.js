...
class Superwar {
    ...
    isFight = () => {
        // Type your code here
        if(this.players.filter(player => player.selected && 
            player.strength > 0).length == 2){
                return 'clash';
            }
            else{
                return 'peace';
    ...
    totalStrength = (type) => {
        ...
        return this.players.filter(player => player.type == type).reduce((totalStrength, player) => totalStrength + player.strength, 0)
    }
